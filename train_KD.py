...
def parse_args():
    ...
    parser.add_argument('--alpha', type=float, default=0.7, help='Weight for distillation loss (1-alpha for task loss)')
    ...
    parser.add_argument('--kd_mode', type=int, default=0, 
                        choices=[0, 1, 2, 3, 4],
                        help='Knowledge distillation mode: 0--4 for no_kd, logits KD, input_features KD, output_features KD, both_features KD ')
    parser.add_argument('--feature_loss_weight', type=float, default=1.0, help='Weight for feature distillation loss')
...
class DistillationLoss(nn.Module):
    """
    Enhanced Knowledge Distillation Loss supporting both logits and feature-based distillation
    """
    def __init__(self, task_criterion, temperature=4.0, alpha=0.7, kd_mode='logits', 
                 feature_loss_weight=1.0, input_feature_weight=0.5, output_feature_weight=0.5):
        ...
        self.input_feature_weight = input_feature_weight
        self.output_feature_weight = output_feature_weight
        self.kl_div = nn.KLDivLoss(reduction='batchmean')
        self.mse_loss = nn.MSELoss()

    def feature_distillation_loss(self, student_features, teacher_features, shape_mapping=None):
        """
        ...
        if student_features.shape == teacher_features.shape:     
            ...
            student_features_mapping = shape_mapping(student_features)
            feature_loss = self.mse_loss(student_features_mapping, teacher_features)
        return feature_loss
    ...
    def forward(self, student_logits, teacher_logits, targets, 
                ...
                input_shape_mapping=None, output_shape_mapping=None):
        """
        Args:
            ...
            output_shape_mapping: shape mapping network for output features
        """
        ...
        if self.kd_mode == 0:
            ...
            distillation_loss = self.feature_distillation_loss(student_input_features, teacher_input_features, input_shape_mapping)
            distillation_loss *= self.feature_loss_weight
            ...
            distillation_loss = self.feature_distillation_loss(student_output_features, teacher_output_features, output_shape_mapping)
            distillation_loss *= self.feature_loss_weight
            ...
            input_feature_loss = self.feature_distillation_loss(student_input_features, teacher_input_features, input_shape_mapping)
            output_feature_loss = self.feature_distillation_loss(student_output_features, teacher_output_features, output_shape_mapping)
            
            distillation_loss = (self.input_feature_weight * input_feature_loss + 
                               self.output_feature_weight * output_feature_loss) * self.feature_loss_weight
...
def train_model(teacher_model, student_model, dataloaders, args, optimizer, scheduler, device, save_path, input_shape_mapping=None, output_shape_mapping=None):
    ...
    distillation_criterion = DistillationLoss(
        ...
        input_feature_weight=args.input_feature_weight,
        output_feature_weight=args.output_feature_weight
    )
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        with tqdm(dataloaders['train'], unit="batch", file=sys.stdout) as tepoch:
            for i, (img, beam, label) in enumerate(tepoch, 0):
                ...
                img = img.unsqueeze(2)
                image_batch_student = torch.cat([img[:,1-args.seq_length_student:, ...], torch.zeros_like(img[:, :args.num_pred, ...])], dim=1).to(device)
                image_batch_teacher = torch.cat([img[:,1-args.seq_length_teacher:, ...], torch.zeros_like(img[:, :args.num_pred, ...])], dim=1).to(device)
                ...
                OutFeature_s_valid = student_out_features[:,:args.seq_length_student-1,:] # extract valid output features
                ...
                if args.kd_mode != 0:
                    with torch.no_grad():
                        ...
                        OutFeature_t_valid = teacher_out_features[:,:args.seq_length_teacher-1,:] # extract valid output features
                else:
                    # Create dummy teacher outputs for no_kd mode
                    teacher_outputs = torch.zeros([args.batch_size, args.seq_length_teacher-1, args.num_classes], device=device)
                    InFeature_t_valid = torch.zeros([args.batch_size, args.seq_length_teacher-1, args.num_classes], device=device)
                    OutFeature_t_valid = torch.zeros([args.batch_size, args.seq_length_teacher-1, args.num_classes], device=device)
                ...
                total_loss, task_loss, distill_loss = distillation_criterion(
                    ...
                    input_shape_mapping, output_shape_mapping
                )
                
                total_loss.backward()

                # Clip gradients for student model and shape mapping networks
                all_params = list(student_model.parameters())
                if input_shape_mapping is not None:
                    all_params.extend(list(input_shape_mapping.parameters()))
                if output_shape_mapping is not None:
                    all_params.extend(list(output_shape_mapping.parameters()))
...
def validate_model(epoch, model, dataloader, args, device, save_path):
    ...
    for i, (img, beam, label) in enumerate(dataloader, 0):
        ...
        with torch.no_grad():
            outputs, _, _ = model(image_batch_student, beam_opt)  # Unpack the three outputs
...
def main():
    ...
    for file in ['train_KD.py', 'DataFunc.py', 'model.py']:
    ...
    student_model = StudentModalityNet(args.feature_size, args.num_classes, GRU_PARAMS)
    ...
    output_shape_mapping = None
    
    if args.seq_length_student != args.seq_length_teacher and args.kd_mode in [2, 3, 4]:
        if args.kd_mode == 2:
            # Only input features KD - create single mapping for input features
            input_shape_mapping = ShapeMapping(args.seq_length_student-1, args.seq_length_teacher-1, args.feature_size)
            input_shape_mapping.to(device)
            print("=====Input shape mapping model created for mode 2 (input features only)=====")
            
        elif args.kd_mode == 3:
            # Only output features KD - create single mapping for output features
            output_shape_mapping = ShapeMapping(args.seq_length_student-1, args.seq_length_teacher-1, args.gru_hidden_size)
            output_shape_mapping.to(device)
            print("=====Output shape mapping model created for mode 3 (output features only)=====")
            
        elif args.kd_mode == 4:
            # Both features KD - create separate mappings for input and output features
            input_shape_mapping = ShapeMapping(args.seq_length_student-1, args.seq_length_teacher-1, args.feature_size)
            output_shape_mapping = ShapeMapping(args.seq_length_student-1, args.seq_length_teacher-1, args.gru_hidden_size)
            input_shape_mapping.to(device)
            output_shape_mapping.to(device)
            print("=====Separate input and output shape mapping models created for mode 4 (both features)=====")
    
    elif args.kd_mode in [2, 3, 4]:
        print("=====No shape mapping needed - student and teacher have same sequence length=====")
    else:
        print("=====No shape mapping needed=====")

    # Print model summary using student model
    image_input = torch.randn(args.batch_size, args.seq_length_student-1, 1, 224, 224).to(device)
    ...
    with open(os.path.join(save_directory, 'params.txt'), 'a') as f:
        ...
        if input_shape_mapping is not None:
            input_mapping_params = sum(p.numel() for p in input_shape_mapping.parameters())
            trainable_params += input_mapping_params
            total_params += input_mapping_params
        if output_shape_mapping is not None:
            output_mapping_params = sum(p.numel() for p in output_shape_mapping.parameters())
            trainable_params += output_mapping_params
            total_params += output_mapping_params
        ...
        if input_shape_mapping is not None:
            f.write(f"Input shape mapping: {input_mapping_params:,} parameters\n")
        if output_shape_mapping is not None:
            f.write(f"Output shape mapping: {output_mapping_params:,} parameters\n")
        f.write(f"\nTraining Mode:\n")
    ...
    # Setup optimizer and scheduler for student model and shape mapping networks
    optimizer_params = list(student_model.parameters())
    if input_shape_mapping is not None:
        optimizer_params.extend(list(input_shape_mapping.parameters()))
    if output_shape_mapping is not None:
        optimizer_params.extend(list(output_shape_mapping.parameters()))
    ...
    train_acc_hist, train_loss_hist, test_acc_hist, test_loss_hist, lrs, train_task_loss_hist, train_distill_loss_hist = train_model(
        teacher_model, student_model, dataloaders, args, optimizer, scheduler, device, save_directory, input_shape_mapping, output_shape_mapping
    )
